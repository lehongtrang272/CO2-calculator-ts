...
const mockTransportData = [
  ...
  {
    identifier: 'transport2',
    factor: 3,
    target_country: 'IT',
    origin_country: 'DE',
  },
  {
    identifier: 'transport3',
    factor: 4,
    target_country: 'FR',
    origin_country: 'IT',
  },
...
const mockFetchFootprintData =
  require('../clients/footprintClient').fetchFootprintData;
const mockFetchTransportData =
  require('../clients/transportClient').fetchTransportData;
describe('calculateFootprint', () => {
  ...
  it('should throw InvalidFootprintError for invalid footprint', async () => {
    ...
    await expect(
      calculateFootprint('invalid_footprint', 'transport1', 'IT')
    ).rejects.toThrow('Invalid footprint or transport type');
  });
  ...
  it('should throw InvalidTransportationError for invalid transport', async () => {
    ...
    await expect(
      calculateFootprint('footprint1', 'invalid_transport', 'IT')
    ).rejects.toThrow('Invalid transport type or target country');
  });
});
